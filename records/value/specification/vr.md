...
This is not a hypothetical. It is the failure demonstrated in [`conformance/FIX-003`](../fixtures/FIX-003-two-commitments-not-one.md), where the beneficiary clause is the *only* signal separating two workstream benefits written to the same template by the same author in the same document. Anything grouping on similarity merges them.
...
This specification versions independently of [`spec.md`](../../decision/specification/tdr.md), which it tracked only until v1.3.0.
