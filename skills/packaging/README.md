...
| [`runtime-evidence.md`](runtime-evidence.md) | **What was verified about each runtime, from which source, on what date** — so upstream change is detectable rather than discovered |
| [`canonical/`](canonical/) | The collected skills, the catalogue enumerated from the manifests, and the bundle's provenance. Generated |
| `claude-code/` · `codex/` · `gemini-cli/` · `copilot/` · `ollama-agent/` | Per-surface install and invocation, generated from verified evidence |
...
Everything under `canonical/` and the surface directories is generated by
[`mtdr_packaging`](../../mtdr_packaging/) and checked for drift on every suite run. Edit the manifests
or the skills, then regenerate — never the bundle.

**The rule that governs everything here:** adapters translate packaging and invocation, never
semantics. A runtime that could only be supported by changing the meaning of a canonical artefact is
unsupported, and that is not a reason to change the standard.
