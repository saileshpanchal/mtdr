...
### Added
- **Five kinds of scope** (spec §4.4) — applicability, authority, **access**, **disclosure** and impact are five different questions, and conflating them is a common error. An exception is an applicability boundary, not an access rule; a restricted decision is not a narrowly-applicable one. Access and disclosure are separated deliberately: that someone may *read* a decision does not settle whether it may be *repeated*, to whom, or when. The disclosure condition is usually **a decision in its own right** — "communicate to affected customers only once the authorised communication has been issued" has an owner, an authority and a trigger — so the standard exists to record it, not to apply it. That is also the honest way to split what looks like one decision: closing a site, beginning preparation, communicating after a defined event and offering redeployment are four decisions with different owners, timings and audiences.
...
- **What this format does not yet compute** (spec §11) — `decision_date` says when a judgement was made, not when it took force; there is no effective-from, effective-until, suspended state or jurisdictional applicability. A body of TDRs therefore *supports* the current position without computing it. Time is also more than one date: a change carries distinct moments — decided, governing internally, preparation permitted, communication authorised, communication issued, generally disclosable — which rarely coincide and are frequently separate decisions. An event-based condition is not a date: if the communication slips, the event has not happened. Also states the governed-vs-realised distinction: what was decided, implemented, controlled and actually happened are four different things.
- **An explicit allocation of what the format does not do** ([TDR-0015](decisions/TDR-0015-visibility-and-effectiveness.md)) — computing what is in force for a participant, assembling authorised views, inference and aggregation control, and participant/actor identity are each allocated to the layer an adopter builds above the register. The relevant disciplines are mature but disjoint, and no widely adopted standard unites them around an organisation's decisions — which is why the boundary is written down rather than assumed. A format that implied it covered them would be misleading; the failure mode is an adopter believing a field is a control.
