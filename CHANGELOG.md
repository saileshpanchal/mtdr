...
- **A three-layer boundary contract** ([TDR-0015](decisions/TDR-0015-visibility-and-effectiveness.md)) — a table allocating each responsibility across the standard, the register or store, and the downstream platform: capture; enforcing direct access and compartments; resolving identity, mandate and entitlement; computing applicability and effectiveness; constructing authorised projections; controlling use, disclosure, aggregation and inference. With two cautions that are easy to get wrong: **a record's `status` is never proof of operability, authority or permission to disclose**, and **capture confers nothing** — recording a disclosure condition does not authorise a disclosure. The relevant disciplines are mature but disjoint, and this review did not identify, as of July 2026 and within the scope reviewed, a single adopted standard uniting them around an organisation's decisions — which is why the boundary is written down rather than assumed.
